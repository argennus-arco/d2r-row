...
const runeWords = [
        ...
        { name: "열정 (Passion)", alias: ["패션"], runes: ["돌", "오르트", "엘드", "렘"], level: 43, sockets: 4, types: [ITEM_TYPES.SWORD, ITEM_TYPES.AXE, ITEM_TYPES.MACE, ITEM_TYPES.POLEARM, ITEM_TYPES.SPEAR, ITEM_TYPES.STAFF, ITEM_TYPES.WAND, ITEM_TYPES.SCEPTER, ITEM_TYPES.CLAW, ITEM_TYPES.DAGGER], effects: "공격 속도 +25%, 피해 +160-210% 증가, 명중률 보너스 50-80%, 언데드에게 주는 피해 +75%, 언데드에 대한 명중률 +50, 번개 피해 1-50 추가, +1 광폭화, +1 열의, 적중 시 대상 실명 +10, 적중 시 괴물 도주 +25%, 괴물에게서 얻는 금화 75% 증가, 3레벨 울버린의 심장(충전 12회)" },
        ...
        { name: "신중 (Prudence)", alias: ["프루던스", "현명"], runes: ["말", "티르"], level: 49, sockets: 2, types: [ITEM_TYPES.ARMOR], note: "에테리얼 갑옷에 홈 제작 추천<br>(파괴불가 없음, 내구 회복 있음)", effects: "타격 회복 속도 +25%, 방어력 +140-170% 증가, 모든 저항 +25-35, 피해 3 감소, 마법 피해 17 감소, 적 처치 시 마나 +2, 시야 +1, 내구도 1 회복(4초마다)" },
        ...
        { name: "서약 (Oath)", alias: ["오쓰", "허약", "빈약", "쇠약"], runes: ["샤엘", "풀", "말", "룸"], level: 49, sockets: 4, types: [ITEM_TYPES.SWORD, ITEM_TYPES.AXE, ITEM_TYPES.MACE], note: "에테리얼 재료 필수 (파괴 불가)", effects: "파괴 불가, 타격 시 30% 확률로 20레벨 뼈 영혼 시전, 공격 속도 +50%, 피해 +210-340% 증가, 악마에게 주는 피해 +75%, 악마에 대한 명중률 +100, 괴물 회복 저지, 마력 +10, 마법 흡수 +10-15, 16레벨 울버린의 심장(충전 20회), 17레벨 강철 골렘(충전 14회)" },
        ...
        // === Level 55 ===
        { name: "침묵 (Silence)", alias: ["사일런스"], runes: ["돌", "엘드", "헬", "이스트", "티르", "벡스"], level: 55, sockets: 6, types: CATEGORIES.WEAPON_ALL, effects: "모든 기술 +2, 공격 속도 +20%, 타격 회복 속도 +20%, 피해 +200% 증가, 언데드에게 주는 피해 +75%, 언데드에 대한 명중률 +50, 적중당 마나 11% 훔침, 적중 시 대상 실명 +33, 적중 시 괴물 도주 +25%, 모든 저항 +75, 적 처치 시 마나 +2, 매직 아이템 발견 확률 30% 증가, 착용 조건 -20%" },
        ...
        // === Level 57 ===
        { name: "소집 (Call to Arms)", alias: ["콜투", "콜투암스", "CTA"], runes: ["앰", "랄", "말", "이스트", "오움"], level: 57, sockets: 5, types: CATEGORIES.WEAPON_ALL, note: "크리스탈 소드/프레일 권장", effects: "모든 기술 +1, 공격 속도 +40%, 피해 +250-290% 증가, 화염 피해 5-30 추가, 적중당 생명력 7% 훔침, +2-6 전투 명령, +1-6 전투 지시, +1-4 전투의 함성, 괴물 회복 저지, 생명력 회복 +12, 매직 아이템 발견 확률 30% 증가" },
        ...
        { name: "추방 (Exile)", alias: ["망명", "엑자일"], runes: ["벡스", "오움", "이스트", "돌"], level: 57, sockets: 4, types: [ITEM_TYPES.SHIELD, ITEM_TYPES.PALADIN_SHIELD], subType: "성기사 전용", note: "에테리얼 전용 방패 필수 (내구 회복)", effects: "타격 시 15% 확률로 5레벨 생명력 추출 시전, 장착 시 13-16레벨 인내 오라 효과 적용, 공격 오라 +2 (성기사 전용), 막기 속도 +30%, 대상 빙결, 방어력 +220-260% 증가, 생명력 회복 +7, 최대 냉기 저항 +5%, 최대 화염 저항 +5%, 매직 아이템 발견 확률 25% 증가, 내구도 1 회복(4초 마다)" },
        
        // === Level 59 ===
        { name: "인내 (Fortitude) - 무기", alias: ["인내"], runes: ["엘", "솔", "돌", "로"], level: 59, sockets: 4, types: CATEGORIES.WEAPON_ALL, effects: "피격 시 20% 확률로 15레벨 냉기 갑옷 시전, 시전 속도 +25%, 피해 +300% 증가, 최소 피해 +9, 명중률 +50, 치명적 공격 +20%, 적중 시 괴물 도주 +25%, 방어력 +200% 증가, 생명력 +(캐릭터 레벨 × 1)-(캐릭터 레벨 × 1.5), 모든 저항 +25-30, 받는 피해의 +12%만큼 마나 회복, 시야 +1" },
        ...
        { name: "슬픔 (Grief)", alias: ["깊은고뇌", "고뇌", "그리프"], runes: ["에드", "티르", "로", "말", "랄"], level: 59, sockets: 5, types: [ITEM_TYPES.SWORD, ITEM_TYPES.AXE], effects: "타격 시 35% 확률로 15레벨 맹독 시전, 공격 속도 +30-40%, 피해 +340-400, 대상의 방어력 무시, 대상의 방어력 -25%, 악마에게 주는 피해 +(캐릭터 레벨 × 1.875)%, 화염 피해 5-30 추가, 적의 독 저항 -20-25%, 치명적 공격 +20%, 괴물 회복 저지, 적 처치 시 마나 +2, 적 처치 시 생명력 +10-15" },
        ...
        { name: "영원 (Eternity)", alias: ["이터니티", "불멸"], runes: ["앰", "베르", "이스트", "솔", "수르"], level: 63, sockets: 5, types: CATEGORIES.MELEE, effects: "파괴 불가, 피해 +260-310% 증가, 최소 피해 +9, 적중당 생명력 7% 훔침, 강타 확률 +20%, 적중 시 대상 실명, 대상 감속 33%, 마나 재생 16%, 생명력 회복 +16, 빙결되지 않음, 매직 아이템 발견 확률 30% 증가, 8레벨 부활(충전 88회)" },
        ...
        // === Level 65 ===
        { name: "수수께끼 (Enigma)", alias: ["수수", "에니그마"], runes: ["자", "아이드", "베르"], level: 65, sockets: 3, types: [ITEM_TYPES.ARMOR], effects: "모든 기술 +2, 이동 속도 +45%, +1 순간이동, 방어력 +750-775, 힘 +0-74 (캐릭터 레벨에 비례), 최대 생명력 5% 증가, 받는 물리 피해 8% 감소, 적 처치 시 생명력 +14, 받는 피해의 +15%만큼 마나 회복, 매직 아이템 발견 확률 1-99% 증가 (캐릭터 레벨에 비례)" },
        { name: "불사조 (Phoenix) - 무기", alias: ["피닉스"], runes: ["벡스", "벡스", "로", "자"], level: 65, sockets: 4, types: CATEGORIES.WEAPON_ALL, effects: "레벨 상승 시 100% 확률로 40레벨 불길 시전, 타격 시 40% 확률로 22레벨 화염폭풍 시전, 장착 시 10-15레벨 속죄 오라 효과 적용, 피해 +350-400% 증가, 대상의 방어력 무시, 적중당 마나 14% 훔침, 적의 화염 저항 -28%, 치명적 공격 +20%, 원거리 공격 방어력 +350-400, 화염 흡수 +15-21" },
        ...
        // === Level 67 ===
        { name: "정의의 손길 (Hand of Justice)", alias: ["정손", "핸드오브저스티스", "HOJ"], runes: ["수르", "참", "앰", "로"], level: 67, sockets: 4, types: CATEGORIES.WEAPON_ALL, effects: "레벨 상승 시 100% 확률로 36레벨 불길 시전, 사망 시 100% 확률로 48레벨 운석 낙하 시전, 장착 시 16레벨 신성한 불꽃 오라 효과 적용, 공격 속도 +33%, 피해 +280-330% 증가, 대상의 방어력 무시, 적의 화염 저항 -20%, 적중당 생명력 7% 훔침, 치명적 공격 +20%, 적중 시 대상 실명, 대상 빙결 +3" },
        ...
        { name: "긍지 (Pride)", alias: ["자존심", "프라이드", "존심"], runes: ["참", "수르", "이오", "로"], level: 67, sockets: 4, types: [ITEM_TYPES.POLEARM], effects: "피격 시 25% 확률로 17레벨 화염벽 시전, 장착 시 16-20레벨 집중 오라 효과 적용, 명중률 보너스 260-300%, 악마에게 주는 피해 +(캐릭터 레벨 × 1)%, 번개 피해 50-280 추가, 치명적 공격 +20%, 적중 시 대상 실명, 대상 빙결 +3, 활력 +10, 생명력 회복 +8, 괴물에게서 얻는 금화 (캐릭터 레벨 × 1.875)% 증가" },
        ...
        { name: "탈태 (Metamorphosis)", alias: ["메타모포시스"], runes: ["이오", "참", "팔"], level: 67, sockets: 3, types: [ITEM_TYPES.HELM, ITEM_TYPES.DRUID_PELT], subType: "드루이드 전용 투구", effects: "늑대인간으로 타격 시 180초 간 징표 효과 부여 (명중률 보너스 30%, 최대 생명력 +40%), 곰인간으로 타격 시 180초 간 징표 효과 부여 (공격 속도 +25%, 받는 물리 피해 20% 감소), 변신 기술 +5 (드루이드 전용), 강타 확률 25%, 방어력 +50-80%, 힘 +10, 활력 +10, 모든 저항 +10, 빙결되지 않음" },
        
        // === Level 69 ===
        { name: "죽어가는 자의 숨결 (Breath of the Dying)", alias: ["죽숨", "브레스오브더다잉", "BOTD"], runes: ["벡스", "헬", "엘", "엘드", "조드", "에드"], level: 69, sockets: 6, types: CATEGORIES.WEAPON_ALL, note: "에테리얼 재료 필수", effects: "적 처치 시 50% 확률로 20레벨 맹독 확산 시전, 파괴 불가, 공격 속도 +60%, 피해 +350-400% 증가, 대상의 방어력 -25%, 명중률 +50, 언데드에게 주는 피해 +200%, 언데드에 대한 명중률 +50, 적중당 마나 7% 훔침, 적중당 생명력 12-15% 훔침, 괴물 회복 저지, 모든 능력치 +30, 시야 +1, 착용 조건 -20%" },
